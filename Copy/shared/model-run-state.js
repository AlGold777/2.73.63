...
(function initModelRunState(root) {
  ...
  const TERMINAL_STATUSES = Object.freeze([...SUCCESS_STATUSES, ...FAILURE_STATUSES]);
  ...
  function isTerminalRunState(stateOrEntry = {}) {
    ...
    return Boolean(
      ...
      || stateOrEntry?.finalStatusRecorded
      || stateOrEntry?.finalStatus
  ...
  function shouldBlockTransition(entry = {}, transition, payload = {}) {
    ...
    if (transition === TRANSITIONS.ANSWER_CANDIDATE_ACCEPTED && !payload.manualRecovery && !payload.allowTerminalUpgrade) {
  ...
  function applyModelRunTransition(entry, transition, payload = {}) {
    ...
    if (block.block) {
      ...
      return { ok: true, applied: false, reason: block.reason, state: deriveModelRunState(entry) };
    }
    ...
    switch (normalizedTransition) {
      ...
      case TRANSITIONS.RESET:
        ...
        return { ok: true, applied: true, reason: 'reset', state: entry.modelRunState };
      default:
        break;
    }
    ...
    return { ok: true, applied: true, reason: block.reason || 'accepted', state };
  }
