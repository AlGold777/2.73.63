...
function maybeDeferStreamingFinalization(llmName, answer, metaObj, answerHtml, normalizedAnswer) {
  ...
  chrome.scripting.executeScript({
    ...
    if (state?.active && elapsedMs < DEFER_STREAM_FINAL_MAX_MS) {
    ...
    handleLLMResponse(
      ...
      null,
      {
        ...
        responseMeta: {
          ...
          completionReason: elapsedMs >= DEFER_STREAM_FINAL_MAX_MS ? 'defer_max_elapsed' : 'generation_inactive'
        }
      },
...
function submitAnswerCandidate(llmName, entry, candidate = {}, context = {}) {
  ...
  if (entry && self.ModelRunState?.applyModelRunTransition) {
    self.ModelRunState.applyModelRunTransition(entry, transition, {
...
function handleLLMResponse(llmName, answer, error = null, meta = null, answerHtml = '') {
  ...
  if (entry) {
    ...
    if (!alreadyFinalized || allowTerminalUpgrade || allowManualTerminalOverride) {
      ...
      entry.statusReason = finalReason;
    } else {
      const lockedStatus = String(entry.finalStatus || entry.status || 'ERROR').toUpperCase();
      entry.status = lockedStatus;
      entry.statusReason = entry.statusReason || 'terminal_locked';
    }
    entry.responseMeta = responseMeta;
    if (finalStatus === 'SUCCESS' || finalStatus === 'PARTIAL' || finalStatus === 'STREAM_TIMEOUT_HIDDEN') {
      entry.hardStopDeferredAt = 0;
      entry.hardStopDeferredDispatchId = null;
    ...
    if (self.ModelRunState?.applyModelRunTransition) {
      ...
      self.ModelRunState.applyModelRunTransition(entry, finalTransition, {
        status: finalStatus,
        reason: finalReason,
        answerLength: trimmedAnswer.length,
  ...
  if (jobState.responsesCollected >= jobState.session.totalModels) {
    ...
    const stalled = Object.entries(jobState.llms || {})
      .filter(([_, entry]) => entry && !TERMINAL_STATUSES.includes(entry.status))
      .map(([name]) => name);
