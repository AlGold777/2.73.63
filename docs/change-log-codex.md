...
### 2026-06-07 00:00 CEST — Model Favorites Sync (WIP)
...
- Для чего: вернуть toolbar выделения на главной странице в правильный координатный режим. Изменение: `#responseSelTb` снова имеет `position: fixed` с высоким `z-index`, чтобы floating toolbar не наследовал absolute-позиционирование от общего `.debate-sel-toolbar` и не улетал далеко вверх относительно выделенного текста.
- Для чего: сделать Favourite свёрнутым по умолчанию и держать фокус на последней записи. Изменение: `favorite-panel` теперь стартует в collapsed layout с минимальной высотой 150px и внутренним скроллом, а после рендера и при double-click панель автоматически прокручивается к нижней записи; при раскрытии `llm-panel-expanded` скролл также остаётся внизу.
...
- Для чего: вернуть служебные кнопки запроса на правую сторону нижней строки и скрыть заголовок LLM Stream по умолчанию. Изменение: `+`, copy и trash снова стоят справа от иконок управления перед кнопкой отправки, а `modifiers-section` и `pro-toggle-wrapper` запускаются с `hidden` и скрываются через `display:none !important`; сам prompt-container получил `debate-composer` layout для центрирования до отправки запроса.
- Для чего: убрать растягивание главного prompt textarea до высоты всей панели. Изменение: `#prompt-input` на главной странице больше не flex-grow’ится как `1 1 auto`, а живёт как компактное textarea с auto-grow только до рабочей высоты, поэтому prompt-блок снова сопоставим по высоте с pipeline field.
- Для чего: снять базовый `min-height: 150px` у main-page prompt container. Изменение: `.prompt-container.prompt-sandwich` на главной странице больше не наследует full-bleed высоту, поэтому prompt-блок схлопывается до компактной высоты 86px вместо 150px.
- Для чего: вертикально центрировать компактный prompt-блок главной страницы как на pipeline. Изменение: `app-main .input-section` получил отдельный vertical-centering layout с высотой `calc(100dvh - 172px)`, поэтому блок запроса стоит в середине экрана, но сохраняет компактную высоту.
- Для чего: выровнять высоту main prompt блока с pipeline prompt блоком. Изменение: главный `.prompt-container.prompt-sandwich` получил ту же внешнюю высоту `calc(100dvh - 206px)`, что и pipeline field, поэтому размеры блоков запроса совпадают.
- Для чего: вернуть main prompt к компактному centered layout, как в исходной форме главной страницы. Изменение: `.prompt-container.prompt-sandwich:not(.debate-composer)` снова имеет фиксированную высоту `86px` и центрируется через `app-main .input-section`, поэтому блок запроса визуально совпадает с pipeline-like compact card без растягивания по высоте.
- Для чего: выровнять высоту главного prompt-блока по pipeline moderator input. Изменение: `.prompt-container.prompt-sandwich:not(.debate-composer)` теперь имеет фиксированную высоту `76px`, то есть совпадает по высоте с `#modTa`-контейнером на pipeline-page и не раздувается на лишнюю строку.
- Для чего: увеличить главный prompt textarea до двух строк без разъезда layout. Изменение: `.prompt-container.prompt-sandwich:not(.debate-composer)` теперь имеет высоту `122px`, а `#prompt-input` на главной странице занимает `66px`, чтобы textarea визуально был двухстрочным и не ломал центрирование.
...
- Для чего: покрыть новый cross-page flow тестами. Изменение: добавлен `tests/model-selection-toolbar.test.js` для model-page selection-toolbar, а `tests/results-debate-favorites.test.js` обновлён под текущие swatch-цвета toolbar.
...
### 2026-06-05 00:00 CEST — v2.74.46
...
- Для чего: зафиксировать PR2 regression contract. Изменение: `tests/finalization-evidence.test.js` проверяет классификацию `transport_error_after_submit`, mapping в final status и сохранение `failureClass` в finalization evidence.
- Для чего: зафиксировать выпуск. Изменение: версия расширения обновлена до `2.74.46`. Файл: `manifest.json`.
- Проверка: `npx jest --config tests/jest.config.js --runInBand` (`23/23` suites, `109/109` tests).

### 2026-06-04 18:30 CEST — v2.74.45
...
- Для чего: зафиксировать новый invariant тестом. Изменение: `tests/pipeline-fsm.test.js` проверяет, что обычный duplicate final остаётся заблокированным, а recovered success после `NO_SEND` принимается только с `allowRecoveredFinal=true`.
- Для чего: зафиксировать выпуск. Изменение: версия расширения обновлена до `2.74.45`. Файл: `manifest.json`.
- Проверка: `npx jest --config tests/jest.config.js --runInBand` (`23/23` suites, `107/107` tests).
